...
logo = "https://telegra.ph/file/252f9c8a46b29ee1bca1e.jpg"

@bot.on(d3vil_cmd(pattern="imdb ?(.*)"))
@bot.on(sudo_cmd(pattern="imdb ?(.*)", allow_sudo=True))
async def _(event):
    reply_to = await reply_id(event)
    d3vl_ = await eor(event, "`Processing ...`")
    try:
        d3vil = event.pattern_match.group(1)
        await d3vl_.edit("__Searching for__ `{}`".format(d3vil))
        # Credits to catuserbot.
        # Ported to d3vilbot and beautifications by @D3_krish.
        movies = imdb.search_movie(d3vil)
...
<b>Director :</b> <code>{mov_director}</code>"""
        ...
        paste = await telegraph_paste(f"IMDb Movie Result for “ {d3vil} ”", telegraph_)
        ...
        await d3vl_.edit(f"__No movie found with name {d3vil}.__")
    except Exception as e:
        await d3vl_.edit(f"**Error:**\n__{e}__")
